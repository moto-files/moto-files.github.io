...
function Div(el)
	local alert_type = el.classes[1]
	if alert_type and icons[alert_type] then
		el.classes = {"markdown-alert", "markdown-alert-" .. alert_type}
		local title_html = '<p class="markdown-alert-title">' .. icons[alert_type] .. titles[alert_type] .. '</p>'
		local title_block = pandoc.RawBlock("html", title_html)
		-- Remove the existing title if present
		if el.content[1] and el.content[1].tag == "Div" and el.content[1].classes[1] == "title" then
